...
module_path = os.path.abspath(os.path.join('..', 'tensorpack'))
...
from tensorpack.models import (AvgPooling, BatchNorm,  Conv2D,
                               GlobalAvgPooling)
from tensorpack.models import nonlin
...
class Model(ModelDesc):
    ...
    def build_graph(self, image, label):
        ...
        def residual(name, l, increase_dim=False, first=False):
            ...
            with tf.name_scope(name):
                b1 = l if first else nonlin.BNReLU(l)
                c1 = Conv2D('conv1', b1, out_channel,
                            stride=stride1, activation=nonlin.BNReLU)
        ...
        with argscope([Conv2D, AvgPooling, BatchNorm, GlobalAvgPooling], data_format='channels_first'):
            with argscope(Conv2D, use_bias=False, kernel_size=3, kernel_initializer=tf.compat.v1.variance_scaling_initializer(scale=2.0, mode='fan_out')):
                l = Conv2D('conv0', image, 16, activation=nonlin.BNReLU)
        ...
        wrong = tf.cast(tf.logical_not(tf.nn.in_top_k(
            logits, label, 1)), tf.float32, name='wrong_vector')
    ...
    def optimizer(self):
        ...
        return tf.keras.optimizers.MomentumOptimizer(lr, 0.9)
