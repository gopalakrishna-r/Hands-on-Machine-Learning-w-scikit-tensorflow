...
def check_cifar(dataset):
    ...
    if md5sum != cifar_md5sum:
        logger.error(
            "File `{0}` may be corrupted (wrong md5 checksum). Please delete `{0}` and retry".format(cifar))
        sys.exit(1)
        ...
        for feature, label, coarse_label, batch_name in tqdm(zip_longest(features, labels, coarse_labels, batch_names), total=len(labels), desc="Saving {} images".format(mode)):
            ...
            if args.name_with_batch_index:
                if args.dataset == 'cifar10':
                    filename = '%s_index_%04d.png' % (
                        batch_name, batch_count[batch_name])
                else:
                    filename = '%s_index_%05d.png' % (
                        batch_name, batch_count[batch_name])
            else:
            ...
            if dataset == 'cifar100superclass':
                filepath = os.path.join(
                    output, mode, COARSE_LABELS_LIST[coarse_label], LABELS_LIST[label], filename)
            else:
                filepath = os.path.join(
                    output, mode, LABELS_LIST[label], filename)
